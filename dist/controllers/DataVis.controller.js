...
export const getLastRegister = async (companyId) => {
...
export const getGeneralInsights = async (companyId, startDate, endDate, filters) => {
...
export const getDailyActivity = async (companyId, startDate, endDate, filters) => {
...
// BLOCK VIEWS 
export const getBlockPerformance = async (companyId, from, to, schemaId, filters) => {
    ...
    if (filters.types[0])
        activeBlockTypes.push(BlockType.WORKING);
    if (filters.types[1])
        activeBlockTypes.push(BlockType.REST);
    if (filters.types[2])
        activeBlockTypes.push(BlockType.EXTRA_TIME);
    ...
    const activeDays = [];
    if (filters.days[0])
        activeDays.push(WEEK_DAYS.MONDAY);
    if (filters.days[1])
        activeDays.push(WEEK_DAYS.TUESDAY);
    if (filters.days[2])
        activeDays.push(WEEK_DAYS.WEDNESDAY);
    if (filters.days[3])
        activeDays.push(WEEK_DAYS.THURSDAY);
    if (filters.days[4])
        activeDays.push(WEEK_DAYS.FRIDAY);
    if (filters.days[5])
        activeDays.push(WEEK_DAYS.SATURDAY);
    if (filters.days[6])
        activeDays.push(WEEK_DAYS.SUNDAY);
    ...
    if (activeBlockTypes.includes(BlockType.EXTRA_TIME))
        blockStats = fillGapsWithExtraTime(blockStats);
    ...
    calls.forEach((call, i) => {
        ...
        const targetBlock = blockStats.find(b => callMinutes >= b.startMinutes &&
...
// CALL DURATION
export const getLongCallDistribution = async (companyId, startDate, endDate, filters) => {
...
export const getSeedTimelineHeatmap = async (companyId, year, filters) => {
...
export const getSeedTimelineHeatmapPerDay = async (companyId, targetDate, filters) => {
...
export const getConversionFunnel = async (companyId, startDate, endDate, filters) => {
...
export const getConsistencyHistory = async (goalId, companyId, startDate, endDate, filters) => {
...
export const getAgentsSorted = async (companyId, from, // "YYYY-MM-DD"
to, // "YYYY-MM-DD"
params) => {
     ...
     */
    const agentsData = await prisma.$queryRaw `
    SELECT 
      a.id,
      a.name,
...
///// HELPERS 
export const fillGapsWithExtraTime = (blocks) => {
    const fullDay = [];
