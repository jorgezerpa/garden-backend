...
import { WEEK_DAYS } from "../generated/prisma/client";
 ...
 */
export const handleCallWebhook = async (lastCallId, companyId) => {
    // 0. Check for company
    ...
    if (!agentToThird)
        throw (new Error("Agent has no relation with this third party service"));
    ...
    return await prisma.$transaction(async (tx) => {
        ...
        if (agentToCallee.totalAttemps == 1)
            await tx.funnelEvent.create({ data: { timestamp: new Date(ld.talk_start), agentId: agent.id, callId: call.id, type: "SEED" } });
        if (agentToCallee.totalAttemps > 1)
            await tx.funnelEvent.create({ data: { timestamp: new Date(ld.talk_start), agentId: agent.id, callId: call.id, type: "LEAD" } });
        if (ld.order_ids?.length > 0)
            await tx.funnelEvent.create({ data: { timestamp: new Date(ld.talk_start), agentId: agent.id, callId: call.id, type: "SALE" } });
        return call;
    });
};
// helpers 
 ...
 */
export const mapDateToWeekDayEnum = (dateString) => {
