...
adminRouter.put('/editAgent/:id', checkAgentBelongsToCompany, allowedRoles(["MAIN_ADMIN", "MANAGER"]), async (req, res) => {
    try {
        ...
        if (leadDeskId)
            updateObject.thirdPartyService = { agentServiceIdentifier: leadDeskId, serviceIdentifier: "LEADDESK" };
...
adminRouter.get('/assignation', allowedRoles(["MAIN_ADMIN", "MANAGER"]), async (req, res) => {
    try {
        ...
        const assignations = await GoalsController.getAssignationsByRange(Number(companyId), new Date(from), new Date(to));
...
adminRouter.post('/upsert-assignation', allowedRoles(["MAIN_ADMIN", "MANAGER"]), async (req, res) => {
    try {
        ...
        const result = await GoalsController.upsertGoalAssignation(Number(companyId), date, Number(goalId));
        return res.status(200).json(result);
    }
    catch (err) {
        return res.status(500).json({ error: err.message });
...
adminRouter.delete('/delete-assignation-by-date', checkGoalAssignationBelongsToCompany, allowedRoles(["MAIN_ADMIN", "MANAGER"]), async (req, res) => {
    try {
        ...
        if (companyId && date) {
            const deleted = await GoalsController.deleteGoalAssignationByDate(Number(companyId), new Date(date));
            return res.status(200).json(deleted);
        }
        return res.status(400).json({ error: "Provide either an ID or companyId and date" });
    }
...
async function checkManagerBelongsToCompany(req, res, next) {
    ...
    if (manager.companyId != companyId)
        return res.status(401).json({ error: "Manager does not belogn to company" });
    next();
}
async function checkAgentBelongsToCompany(req, res, next) {
    ...
    if (agent.companyId != companyId)
        return res.status(401).json({ error: "agent does not belogn to company" });
    next();
}
async function checkGoalBelongsToCompany(req, res, next) {
    ...
    if (goal.companyId != companyId)
        return res.status(401).json({ error: "Goal does not belogn to company" });
    next();
}
async function checkGoalAssignationBelongsToCompany(req, res, next) {
    ...
    if (date) {
        const goalsArray = await GoalsController.getAssignationsByRange(companyId, new Date(date), new Date(date));
