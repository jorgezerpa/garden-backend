...
schemaRouter.post('/create', async (req, res) => {
    try {
        ...
        if (blocks.length == 0)
            return res.status(400).json({ error: "Must send at least 1 block" });
...
schemaRouter.get('/list', async (req, res) => {
    try {
        const companyId = req.user?.companyId;
        if (!companyId)
            return res.status(404).json({ error: "missing companyId" });
...
schemaRouter.get('/assignation', allowedRoles(["MAIN_ADMIN", "MANAGER"]), async (req, res) => {
    try {
        ...
        const assignations = await SchemaController.getAssignationsByRange(Number(companyId), new Date(from), new Date(to));
...
schemaRouter.post('/upsert-assignation', allowedRoles(["MAIN_ADMIN", "MANAGER"]), async (req, res) => {
    try {
        ...
        const result = await SchemaController.upsertSchemaAssignation(Number(companyId), date, Number(schemaId));
        return res.status(200).json(result);
    }
    catch (err) {
        return res.status(500).json({ error: err.message });
